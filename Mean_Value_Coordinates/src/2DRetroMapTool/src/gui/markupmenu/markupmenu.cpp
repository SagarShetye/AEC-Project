...
void FsGui3DMainCanvas::MarkUp_InsertLine_DrawCallBack(void)
{
	if(INSERTMARKUP_DRAGGING==insertMarkUp_Status)
	{
		...
		vtxBuf.Add(insertMarkUp_DragStart);
		vtxBuf.Add(insertMarkUp_CurrentDrag);

		YsGLSLPlain3DRenderer renderer;
		renderer.SetUniformColor(drawingDlg->lineColor->GetColor());
		renderer.DrawVtx(YsGL::LINES,vtxBuf.GetN(),vtxBuf);
	}
}
...
YSRESULT FsGui3DMainCanvas::MarkUp_InsertLine_OnLButtonUp(FsGuiMouseButtonSet,YsVec2i)
{
	if(INSERTMARKUP_DRAGGING==insertMarkUp_Status)
	{
		...
		YsArray <YsVec2i> plg;
		plg.Add(YsVec2i((int)insertMarkUp_DragStart.x(),(int)insertMarkUp_DragStart.y()));
		plg.Add(YsVec2i((int)insertMarkUp_CurrentDrag.x(),(int)insertMarkUp_CurrentDrag.y()));
